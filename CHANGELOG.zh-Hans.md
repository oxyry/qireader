...
- “下一个订阅源”操作会跳过展开状态的文件夹。

# 2026-03-24

- 修复文章列表从旧到新排序时未读标记不显示的问题。
- 修复 E-Ink 主题订阅源列表底部项目被工具栏遮挡问题。
...
- 文章列表：添加了网格视图。
- 文章列表：现在“已读/未读/收藏”标记可点击切换当前状态。
- 添加了一个新的键盘快捷键"U"，用于回退到上一层。一般情况下与左方向和"H"键功能相同，但网格视图无法用方向键回到左边栏，此时可以用"U"切换到左边栏。
- 上/下方向键不再选中上/下条目，而是滚动视图。J/K仍旧是选中上/下条目的功能。
- 左边栏：现在点击当前选中的订阅源会刷新文章列表。
- 当打开订阅源/文章时，对应的网页地址会添加到地址栏"#"号后面。
- PC 端点击工具栏空白区域不再使页面滚动到顶部。
- 改进了大订阅源数量下的加载性能，并减少了内存占用。
- 重置了订阅源内置字体大小，解决了有些微信公众号字体过小的问题。
